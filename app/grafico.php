...
//calculos de cada uno de los biorritmos
    ...
    public function calculaResultado(){
    	...
    	if($fisicoActual<0){
    		$this->resultado[0] = "Tú nivel físico está en negativo, quizás te sientas MUY CANSAO! AAAAAAAAAAAAAAAAAH.";
    	}
    	else if($fisicoActual>0){
    		$this->resultado[0] = "Tú nivel fisico está en positivo, te diría que llames a tu novia, pero como no tienes, prepara tu mano.";
    	}
    	else{
    	...
    	if($intelectoActual<0){
    		$this->resultado[1] = "Tú nivel intelectual está en negativo, CUIDADO CON LOS EXÁMENES DE RAFA!!!.";
    	}
    	else if($intelectoActual>0){
    		$this->resultado[1] = "Tú nivel intelectual está en positivo, quizás es hora de dejar el fortnite y aprobar un examen.";
    	}
    	else{
    	...
    	if($intelectoActual<$intelectoSiguiente){
    		$this->resultado[1] = $this->resultado[1]." En los siguientes días tu nivel intelectual subirá, comprate unas gafitas y hazte el interesante.";
    	}
    	else{
    		$this->resultado[1] = $this->resultado[1]." En los siguientes días tu nivel intelectual bajará, sufres el riesgo de quedarte vegetal.";
    	}
    	//calculo resultados emocional(resultado[2])
    	if($emocionalActual<0){
    		$this->resultado[2] = "Tú nivel emocional está en negativo, no te cortes las venas PLS!.";
    	}
    	else if($emocionalActual>0){
    		$this->resultado[2]= "Tú nivel emocional está en positivo, pero no te emociones que todo lo que sube, baja.";
    	}
    	else{
    	...
    	if($emocionalActual<$emocionalSiguiente){
    		$this->resultado[2] = $this->resultado[2]." En los siguientes días tu nivel emocional subirá, és el momento de tirarle la caña a esa tia que te mola.";
    	}
    	else{
    		$this->resultado[2] = $this->resultado[2]." En los siguientes días tu nivel emocional bajará, ves llamando al psicólogo bro.";
    	}
